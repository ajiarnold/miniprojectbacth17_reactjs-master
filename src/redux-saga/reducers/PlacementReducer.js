...
const PlacementReducer = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case ActionType.EDIT_PLACEMENT_REQUEST:{
      return {
          ...state,
          isLoading: true,
          isRefresh: true
    ...
    case ActionType.EDIT_PLACEMENT_SUCCEED:{
      return applyEditPlacementSucceed(state, action)
    }
    default:
      return state;
  }
...
const applyEditPlacementSucceed = (state, action) => {
  return {
      ...state,
